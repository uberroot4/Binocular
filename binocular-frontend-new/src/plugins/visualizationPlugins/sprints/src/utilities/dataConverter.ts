...
function getDataByStatus(
  ...
  const data: Array<{
    date: number;
    statsBySortingObject: { [status: string]: { count: number } };
  }> = [];
  ...
  for (
    ...
    for (let i = 0; i < sortedIssues.length; i++) {
      if (
        Date.parse(sortedIssues[i].createdAt) >= currTimestamp &&
        Date.parse(sortedIssues[i].createdAt) < nextTimestamp
      ) {
        if (breakdown) {
          totalIssuesPerStatus[IssueStatus.OPEN] += 1;

          obj.statsBySortingObject[IssueStatus.OPEN] = {
            count: totalIssuesPerStatus[IssueStatus.OPEN],
          };
        } else {
      ...
      if (
        Date.parse(<string>sortedIssues[i].closedAt) >= currTimestamp &&
        Date.parse(<string>sortedIssues[i].closedAt) < nextTimestamp
      ) {
        if (breakdown) {
          totalIssuesPerStatus[IssueStatus.OPEN] -= 1;
          obj.statsBySortingObject[IssueStatus.OPEN] = {
  ...
  if (breakdown) {
    palette[IssueStatus.OPEN] = {
      main: chroma('#007AFF').hex(),
      secondary: chroma('#007AFF99').hex(),
    };
  } else {
    palette[IssueStatus.OPENED] = {
      main: chroma('#007AFF').hex(),
      secondary: chroma('#007AFF99').hex(),
    };
    palette[IssueStatus.CLOSED] = {
      main: chroma('#0062cc').hex(),
      secondary: chroma('#0062cc99').hex(),
    };
  }
  ...
  chartData.forEach((dataPoint) => {
    ...
    Object.keys(dataPoint)
      ...
      .forEach((key) => {
        if (
          key.includes(IssueStatus.OPENED) ||
          key.includes(IssueStatus.OPEN)
        ) {
...
function getDataByAuthors(
      ...
      if (
        ...
        if (breakdown) {
          if (IssueStatus.OPENED in obj.statsBySortingObject) {
            ...
            obj.statsBySortingObject[assignee] = {
              OPEN: totalIssuesPerAuthor[assignee],
            };
          }
        } else {
  ...
  if (breakdown) {
    palette['Open Issues ' + UNASSIGNED] = {
      main: '#555555',
      secondary: '#777777',
    };
    palette['Open Issues ' + ACCOUNT_NOT_ASSIGNED] = {
      main: '#444444',
      secondary: '#666666',
    };
  } else {
    palette['Opened Issues ' + UNASSIGNED] = {
      main: '#555555',
      secondary: '#777777',
    };
    palette['Closed Issues ' + UNASSIGNED] = {
      main: '#444444',
      secondary: '#666666',
    };
    palette['Opened Issues ' + ACCOUNT_NOT_ASSIGNED] = {
      main: '#444444',
      secondary: '#666666',
    };
    palette['Closed Issues ' + ACCOUNT_NOT_ASSIGNED] = {
      main: '#333333',
      secondary: '#555555',
    };
  }
  data.forEach((issue) => {
    ...
    if (breakdown) {
      for (const author of authors) {
        palette[
          'Open Issues ' + (author.displayName || author.user.gitSignature)
        ] = {
          main: chroma(author.color.main).hex(),
          secondary: chroma(author.color.secondary).hex(),
        };
        obj['Open Issues ' + (author.displayName || author.user.gitSignature)] =
          0;
      }
      ...
      for (const author of authors) {
        palette[
          'Opened Issues ' + (author.displayName || author.user.gitSignature)
        ] = {
          main: chroma(author.color.main).hex(),
          secondary: chroma(author.color.secondary).hex(),
        };
        palette[
          'Closed Issues ' + (author.displayName || author.user.gitSignature)
        ] = {
      ...
      if (author.user.id in issue.statsBySortingObject) {
        ...
        if (breakdown) {
          if ('Open Issues ' + name in obj) {
            obj['Open Issues ' + name] +=
              issue.statsBySortingObject[author.user.id]?.OPEN || 0;
          } else {
            obj['Open Issues ' + name] =
              issue.statsBySortingObject[author.user.id]?.OPEN || 0;
          }
          ...
          if (
            'Opened Issues ' + name in obj &&
            'Failed Issues ' + name in obj
          ) {
            obj['Opened Issues ' + name] +=
              issue.statsBySortingObject[author.user.id]?.OPENED || 0;
            obj['Closed Issues ' + name] +=
              issue.statsBySortingObject[author.user.id]?.CLOSED || 0;
          } else {
            obj['Opened Issues ' + name] =
              issue.statsBySortingObject[author.user.id]?.OPENED || 0;
            obj['Closed Issues ' + name] =
    ...
    if (UNASSIGNED in issue.statsBySortingObject) {
      if (breakdown) {
        if ('Open Issues ' + UNASSIGNED in obj) {
          obj['Open Issues ' + UNASSIGNED] +=
            issue.statsBySortingObject[UNASSIGNED]?.OPEN || 0;
        } else {
          obj['Opened Issues ' + UNASSIGNED] =
            issue.statsBySortingObject[UNASSIGNED]?.OPENED || 0;
          //-0.001 for stack layout to realize it belongs on the bottom
          obj['Closed Issues ' + UNASSIGNED] =
            issue.statsBySortingObject[UNASSIGNED]?.CLOSED || 0;
        }
        if ('Open Issues ' + ACCOUNT_NOT_ASSIGNED in obj) {
          obj['Open Issues ' + ACCOUNT_NOT_ASSIGNED] +=
            issue.statsBySortingObject[ACCOUNT_NOT_ASSIGNED]?.OPEN || 0;
        } else {
          obj['Opened Issues ' + ACCOUNT_NOT_ASSIGNED] =
            issue.statsBySortingObject[ACCOUNT_NOT_ASSIGNED]?.OPENED || 0;
          //-0.001 for stack layout to realize it belongs on the bottom
          obj['Closed Issues ' + ACCOUNT_NOT_ASSIGNED] =
            issue.statsBySortingObject[ACCOUNT_NOT_ASSIGNED]?.CLOSED || 0;
        }
      } else {
        if (
          'Opened Issues ' + UNASSIGNED in obj &&
          'Failed Issues ' + UNASSIGNED in obj
        ) {
          obj['Opened Issues ' + UNASSIGNED] +=
            issue.statsBySortingObject[UNASSIGNED]?.OPENED || 0;
          //-0.001 for stack layout to realize it belongs on the bottom
        ...
        if (
          'Opened Issues ' + ACCOUNT_NOT_ASSIGNED in obj &&
          'Failed Issues ' + ACCOUNT_NOT_ASSIGNED in obj
        ) {
          obj['Opened Issues ' + ACCOUNT_NOT_ASSIGNED] +=
            issue.statsBySortingObject[ACCOUNT_NOT_ASSIGNED]?.OPENED || 0;
          //-0.001 for stack layout to realize it belongs on the bottom
