...
import { createLineChartData } from '../utilities/dataConverter.ts';
...
import { TestFileSizeEvolutionSettings } from '../settings/settings.tsx';

export interface TestFileSizeEvolutionChartData {
  time: string;
  amountOfTestFiles: number;
}

function Chart<SettingsType extends TestFileSizeEvolutionSettings, DataType>(props: VisualizationPluginProperties<SettingsType, DataType>) {
  // * -----------------------------
  ...
  const [chartData, setChartData] = useState<TestFileSizeEvolutionChartData[]>();
  ...
  useEffect(() => {
    const chartData: TestFileSizeEvolutionChartData[] = createLineChartData(
      commits,
      files,
  ...
  return (
    <div className={'w-full h-full flex justify-center items-center'} ref={props.chartContainerRef}>
      ...
      {dataState === DataState.COMPLETE && (
        <LineChart data={chartData} width={chartWidth} height={chartHeight} dateRange={props.parameters.parametersDateRange} />
      )}
    </div>
  );
}
