...
export function interpolate(a: TestFileSizeEvolutionChartData, b: TestFileSizeEvolutionChartData, targetTime: Date) {
  const t = (targetTime.getTime() - new Date(a.time).getTime()) / (new Date(b.time).getTime() - new Date(a.time).getTime()); // ratio between 0 and 1
  return {
...
// Function to crop data based on the provided min and max time
