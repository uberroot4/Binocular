...
const AuthorSelection: React.FC<AuthorSelectionProps> = ({
  ...
  const handleSelectAuthor = (
    ...
    if (isSelected) {
      ...
      setTempSelectedAuthors(
        tempSelectedAuthors.filter((a) => a.id !== author.id),
      );
    } else {
  ...
  const handleRemoveAuthor = (authorId: string, event: React.MouseEvent) => {
    ...
    setTempSelectedAuthors(
      tempSelectedAuthors.filter((a) => a.id !== authorId),
    );
  };
          ...
          >
            {filteredAuthors.length > 0 ? (
              filteredAuthors.map((author) => {
                const isSelected = tempSelectedAuthors.some(
                  (a) => a.id === author.id,
                );
                return (
                  <div
