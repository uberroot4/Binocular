...
function RadarChart(
  ...
  // Navigation state
  const [currentView, setCurrentView] = useState<"topLevel" | "subpackage">(
    "topLevel",
  );
  ...
  const [individualDeveloperData, setIndividualDeveloperData] = useState<
    Map<string, Package[]>
  >(new Map());
  ...
  useEffect(() => {
    if (
      ...
      dispatch(
        dataSlice.actions.setDateRange(
          properties.parameters.parametersDateRange,
        ),
      );
    }
  ...
  function updateSelectedDevelopers(authorList: AuthorType[]) {
    ...
    setSelectedDevelopers((prev) =>
      prev.filter((developer) =>
        authorList.some(
          (author) => author.user.gitSignature === developer.user.gitSignature,
        ),
      ),
    );
  }
  ...
  const handleAuthorsChange = (authors: AuthorType[]): void => {
    setSelectedDevelopers((prev) =>
      _.isEqual(prev, authors) ? prev : authors,
    );
  };
...
function drawChart(
  ...
  const formatDevelopersData = () => {
    if (currentView === "topLevel") {
      ...
      selectedDevelopers.forEach((developer) => {
        const devData =
          individualDeveloperData.get(developer.user.gitSignature) || [];
      ...
      selectedDevelopers.forEach((developer) => {
        ...
        if (devPackage && devPackage.subpackages) {
          devPackage.subpackages.forEach((subPkg) =>
            allSubpackageNames.add(subPkg.name),
          );
        }
      });
      ...
      return selectedDevelopers.map((developer) => {
        ...
        if (devPackage && devPackage.subpackages) {
          alignedSubpackages = unifiedSubpackages.map((unifiedSub) => {
            const devSub = devPackage.subpackages.find(
              (s) => s.name === unifiedSub.name,
            );
