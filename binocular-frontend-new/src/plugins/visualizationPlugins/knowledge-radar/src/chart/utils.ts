...
export function drawBreadcrumbs(
  ...
  const { radius, dimensions, breadcrumbs, center, selectedDeveloper } =
    options;
  ...
  const breadcrumbGroup = titleGroup
    ...
    .attr(
      "transform",
      `translate(${dimensions.width / 2}, ${center.y + titleY})`,
    );
  ...
  const textWidth = Math.max(
    100,
    Math.min(shortenedBreadcrumbs.length * charWidth, dimensions.width * 0.8),
  );
