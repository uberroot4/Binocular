...
class CommitDao(
...
) : SqlDao<Commit, String>(), ICommitDao {
  init {
    this.setClazz(Commit::class.java)
    this.setRepository(commitRepository)
  }

  override fun findExistingSha(repo: com.inso_world.binocular.cli.entity.Repository, shas: List<String>): Set<Commit> {
  ...
  override fun findAllByRepo(
    repo: com.inso_world.binocular.cli.entity.Repository, pageable: Pageable
  ): Iterable<Commit> {
    if (repo.id == null) throw PersistenceException("Cannot search for repo without valid ID")
  ...
  override fun findHeadForBranch(
    repo: com.inso_world.binocular.cli.entity.Repository, branch: String
  ): Commit? {
    if (repo.id == null) throw PersistenceException("Cannot search for repo without valid ID")
