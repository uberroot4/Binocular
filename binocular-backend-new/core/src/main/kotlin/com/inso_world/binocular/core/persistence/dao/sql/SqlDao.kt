...
class SqlDao<T, I : Serializable> : IDao<T, I> {
    ...
    override fun findAll(pageable: Pageable): List<T> =
        entityManager
            .createQuery("FROM ${clazz.name}", clazz)
    ...
    override fun <S : T> exists(example: Example<S>): Boolean = this.repository.exists(example)

    override fun saveAll(entities: Iterable<T>): Iterable<T> = this.repository.saveAll(entities)
}
