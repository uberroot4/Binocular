...
function ConfigComponent() {
  ...
  const hierarchyState = useSelector((state: any) => 
    state?.visualizations?.changeFrequency?.state?.state || { currentPath: '', hierarchyStack: [] }
  );
  ...
  const config = useSelector((state: any) => 
    state?.visualizations?.changeFrequency?.state?.config || { commitSpan: [new Date(0), new Date()] }
  );
  ...
  function handleStartDateChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    dispatch(setChangeFrequencyConfig({
  ...
  return (
    <div className={styles.configContainer}>
          ...
          <div className={styles.dateRangeControls}>
            <div>
              ...
              <input 
                type="date" 
                id="startDate"
                value={formatDateForInput(commitSpan[0])}
            ...
            <div>
              ...
              <input 
                type="date" 
                id="endDate"
                value={formatDateForInput(commitSpan[1])}
      ...
      <div className={styles.directoryContents}>
        ...
        {isLoading && (
          <div className={styles.emptyDirectory}>Loading data...</div>
        )}
        
        {!isLoading && hierarchyData.length === 0 && (
          <div className={styles.emptyDirectory}>No files or directories found</div>
        )}
